...
const matrixRetrieve = ( branch ) => {
    ...
    oM.SlideMenu = {};
    oM.SlideMenu.existing = typeof SlideMenu;
    oM.SlideMenu.scheduled = "function";
    oM.SlideMenu.referS = [
...
const visualizMenu = () => {
    ...
    const SlideMenu = matrixRetrieve ( "SlideMenu" );
    secuReFerShell ( SlideMenu, containerNav );
    const aTag = document.createElement( "a" );
    aTag.textContent = "menu";
...
const domainReStrict = ( dest ) => {
    const epubJsView = /\bhttps:\/\/alist-org.github.io\/static\/epub.js\/viewer.html\?url=/i;
    const sixty6 = /\bhttps?:\/\/66e\.github\.io/i;
    const jsitor = /\bhttps?:\/\/jsitor\.com/i;
    const msnCn = /\bhttps?:\/\/www\.msn\.cn\/zh-cn\/\w+\/\w+\/[%-\w]+\/[-\w]+/i;
    switch ( true ) {
        case epubJsView.test(dest):
            return "epubJsView";
            break;
        case sixty6.test(dest):
            return "sixty6";
            break;
