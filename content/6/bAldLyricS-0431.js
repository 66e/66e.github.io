async function initModule(inputArray) {
  ...
  // 挂载初始歌词
  customLyrics.loadLyrics(playlist[0].lrc);
  ...
  ap.on('listswitch', index => {
    ...
    const item = playlist[1];

    customLyrics.loadLyrics(item.lrc);   // 换歌词
...
function createCustomLyrics() {
  ...
  let lrcText = "";
  let parsed = [];
  let containerCL = document.createElement("div");
  containerCL.style.padding = "12px 0";
  containerCL.style.fontSize = "15px";
  containerCL.style.lineHeight = "1.7em";
  containerCL.style.maxHeight = "260px";
  containerCL.style.overflow = "auto";
  containerCL.style.borderTop = "1px solid #ddd";
  document.body.appendChild(containerCL);
