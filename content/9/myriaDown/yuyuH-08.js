/**
 * Project: yuyuH - v0.8.1 [Refined & Debugged]
 * Fixes: globalActionId, Mouseover Dot, Bidirectional Slide, 2-Col Volumes
 */
...
(async () => {
    ...
    let globalActionId = 0; // 修复：全局 ID 归位
    ...
    const UI = {
        injectStyles() {
            ...
            s.textContent = `
                #y-dot { position:fixed; bottom:20px; right:20px; width:16px; height:16px; border-radius:50%; background:rgba(0,0,0,0.4); border:2px solid #fff; z-index:2147483647; cursor:pointer; box-shadow: 0 0 8px rgba(0,0,0,0.3); transition: 0.3s; }
                #y-dot:hover { background:#ff00ff; box-shadow: 0 0 12px #ff00ff; }
                #y-menu { position:fixed; bottom:45px; right:20px; width:320px; max-height:75vh; background:#1a1a1a; border-radius:16px; box-shadow:0 20px 50px rgba(0,0,0,0.6); display:none; flex-direction:column; overflow:hidden; z-index:19; font-family:sans-serif; border:1px solid #333; color:#eee; }
                #y-menu.active { display:flex; }
                
                .y-h { position:sticky; top:0; background:#252525; padding:12px 15px; z-index:10; border-bottom:1px solid #333; display:flex; justify-content:space-between; align-items:center; }
                ...
                .y-item { padding:8px; cursor:pointer; font-size:12px; background:#222; border-radius:8px; display:flex; flex-direction:column; align-items:center; border:1px solid #2a2a2a; gap:6px; text-align:center; transition:0.2s; }
                .y-item:hover { border-color:#ff00ff; background:#2a2a2a; }
                .y-item img { height:75px; width:100%; object-fit:cover; border-radius:4px; background:#222 url('${LIBS.loading}') no-repeat center; background-size:16px; }
                ...
                .y-panel-item { cursor: pointer; text-align: center; background:#1a1a1a; padding:10px; border-radius:8px; border:1px solid #222; }
        ...
        render(data, title = "目录", direction = 'forward') {
            ...
            menu.innerHTML = `
                <div class="y-h">
                    ${!isRoot ? `<span class="y-back" id="y-back">⇠ BACK</span>` : '<span style="font-size:10px;letter-spacing:1px;opacity:0.5;">EXPLORE</span>'}
    ...
    dot.onmouseenter = () => toggleMenu(true); // 补齐：悬停弹出
})();