const hYakusho = (function() {
    ...
    build(md) {
        ...
        for (let line of lines) {
            ...
            if (mode === 'gallery' && trim.startsWith('- ')) {
                ...
                // [修改点] 适配新标签名：预先存入锚点索引
                if (tags.simpleAnchor) {
                    state.anchors[tags.simpleAnchor] = node; 
                }
    ...
    activate(nodes, parentTags = {}, depth = 0) {
      ...
      return nodes.map(node => {
          // [关键] 严格保持 0.238 的标签继承顺序，防止 Bypass 逻辑失效
          ...
          // --- 3. 新增: simpleAlias (手术刀式植入) ---
          // 只在当前节点显式定义了 simpleAlias 时触发，不依赖继承
          if (node.tags.simpleAlias && state.anchors[node.tags.simpleAlias]) {
              ...
              // 标记继承来的节点为 endCluster，防止递归
              inherited.forEach(c => {
                  c.tags = { ...c.tags, endCluster: true };
                  c.children = [];
              });
          ...
          // 4. 判定 Bypass (0.238 原版逻辑 - 严禁修改)
          // 这里的逻辑决定了是否有 5 个多余节点被隐藏。必须严格使用 'tags' 变量。
          ...
          if (node.children && node.children.length > 0) {
              // [唯一的新增保护]：仅当当前节点没有 endCluster 标签时才递归
              // 第一本漫画没有 endCluster 标签，所以会正常递归，保持原样。
              // 第二本漫画有 endCluster，会在这里停止递归，保护图片地址。
