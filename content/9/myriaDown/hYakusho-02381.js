...
const EventManager = {
    ...
    render() {
        ...
        const isVolume = node.tags.genSeqPics || 
                     node.tags.endCluster || 
                     (meta.last > 0 && node.children.length === 0);
    ...
    // 调试打印：如果看不到，请检查调用链
    console.log(`[Debug] 节点: ${node.title}`, "标签:", node.tags);

    let urls = [];
    const nodeTags = node.tags || {};
    const meta = this.getMeta(node);

    // 逻辑：如果是终端簇 (endCluster)
    if (nodeTags.endCluster === 'true' || nodeTags.endCluster === true) {
        if (node.children && node.children.length > 0) {
            urls = node.children.map(c => (typeof c === 'string' ? c : c.title).trim());
            console.log(`[Debug] endCluster 提取了 ${urls.length} 个地址`);
        }
    } 
    // 逻辑：如果是序列模式 (第一本)
